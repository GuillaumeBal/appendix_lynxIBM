...
spread.base.gb <- 
  function(landscape, loci = NA_real_, spreadProb = 0.23, persistence = 0,
    ...
    if (!exists("numRetries", envir = .GlobalEnv)) {
      assign("numRetries", rep(0, lenInitialLoci), envir = .GlobalEnv)
    }
    ...
    while (length(loci) & (n <= iterations)) {
      ...
      if (!is.null(neighProbs) | relativeSpreadProb) {
        ...
        if (relativeSpreadProb) {
          ...
          neighIndexToKeep <- unlist(lapply(seq_along(aaa), function(x)
            borrowedResample(aaa[[x]], size = numNeighs[x], prob = rescaledProbs[[x]])))
        } else {
          neighIndexToKeep <- unlist(lapply(seq_along(aaa), function(x)
            borrowedResample(aaa[[x]], size = numNeighs[x])))
        }
      ...
      if (exactSizes) {
        if (all(get("numRetries", inherits = FALSE, envir = .GlobalEnv) < 10)) {
          if (spreadStateExists) {
          ...
          if (any(needPersist)) {
            assign("numRetries", envir = .GlobalEnv,
                   get("numRetries", inherits = FALSE, envir = .GlobalEnv) + needPersist)
            ...
            if (spreadStateExists) {
              ...
              if (needPersistJump) {
                ...
                wh <- mmm[, list(whKeepLoci = borrowedResample(.I, 1)), by = id]$whKeepLoci
              } else {
    ...
    if (returnIndices == 1) {
      ...
      if (exactSizes)
        if (exists("numRetries", envir = .GlobalEnv)) {
          if (sum(allCells$active) == 0) rm("numRetries", envir = .GlobalEnv)
        }
      if  (!(useMatrixVersionSpreads)) {
        #browser(expr = exists("aaaaa"))
